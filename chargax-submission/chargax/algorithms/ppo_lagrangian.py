...
import chex
import optax
from typing import NamedTuple, List, Sequence, Union, Mapping, Literal, Dict
...
def build_ppo_lagrangian_trainer(
    ...
    def train_func(rng=rng):
        ...
        def train_step(runner_state, _):
            ...
            if "logging_data" not in metric:
                metric["logging_data"] = {}
            finished_episodes = metric["returned_episode"] 
            metric["logging_data"] = jax.tree.map(
            ...
            jax.debug.print("rejected_customers: {}", jnp.mean(metric["logging_data"]["rejected_customers"]))
